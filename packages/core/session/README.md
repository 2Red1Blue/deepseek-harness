...
A plugin whose event changes its reconstructed state registers its namespace, schema version, event types, and synchronous payload validators in an effect: `ctx.effect(() => ctx.sessions.registerRequiredExternalEvents(registration))`. It writes those types through `ctx.sessions.appendRequiredExternalEvent(session, type, data)`, which stamps the durable `requiredExternal` identity. The validator receives the immutable detached payload that commits, and a registration changed during validation rejects the append; recursive appends during that boundary are also refused. Cold persistence reads require the exact active registration and revalidate the payload; a missing or mid-read-changed registration refuses the log, and a malformed envelope, marker, or payload is corruption. A validator that returns a Promise is rejected. Restore required records only through `ctx.sessions.prepare({ seedSource: 'persistence', ... })`; its registration must also remain current through construction, while direct `Session.create()` and `Session.fromRestore()` reject the markers. One event type has one active writer registration, so dispose an old version before registering a replacement. This vocabulary is not access control: any code holding `ctx.sessions` can write an active registered type. `session.append()` remains for Harness-owned and live-only extension events and cannot write this marker.
