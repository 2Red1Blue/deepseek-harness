...
事件会改变其重建状态的插件在 effect 中注册命名空间、schema 版本、事件类型与同步载荷校验器：`ctx.effect(() => ctx.sessions.registerRequiredExternalEvents(registration))`。它通过 `ctx.sessions.appendRequiredExternalEvent(session, type, data)` 写入这些类型，后者会写入持久化的 `requiredExternal` 标识。校验器接收的是最终提交的不可变分离载荷，校验期间注册变更会拒绝 append；该边界内的递归 append 也会被拒绝。冷持久化读取要求存在精确且活跃的注册，并重新校验载荷；缺少注册或读取期间注册变更都会拒绝日志，格式错误的信封、标记或载荷属于损坏。返回 Promise 的校验器会被拒绝。必需记录只能通过 `ctx.sessions.prepare({ seedSource: 'persistence', ... })` 恢复，且注册在构造完成前必须保持有效；直接调用 `Session.create()` 与 `Session.fromRestore()` 会拒绝其标记。一个事件类型只能有一个活跃写入注册，因此注册替代版本前必须释放旧版本。此词汇不是访问控制：任何持有 `ctx.sessions` 的代码都可写入活跃已注册类型。`session.append()` 仍用于 Harness 自有事件和仅实时的扩展事件，不能写入此标记。
