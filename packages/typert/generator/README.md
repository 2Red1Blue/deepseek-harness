...
Independent source workspaces can supply `hostConfig`, `clientConfig`, and `additionalPackageRoots` to the analyzer or `WorkspaceTypertGenerator`. The extra roots are source-container directories, resolved relative to the workspace root or supplied as absolute paths. Only directly referenced projects inside the default `packages` directory or an explicitly allowed canonical container are registered; symlinks do not admit projects outside those containers. Aggregate compiler options must resolve the intended source aliases, and callers select the packages they want emitted. Conflicting physical roots for the same package name and compiler face cause analysis to fail, including during focused generation.

`WorkspaceCaches` belongs to one immutable source/configuration snapshot. Registration inventories distinguish workspace, aggregate paths and extra roots; compiler hosts distinguish face and aggregate path. Direct callers of `programHost(face, options, aggregatePath)` must supply the canonical aggregate identity as the third argument; the former two-argument call is not supported by this pre-release API. Callers that edit inputs or create new temporary aggregates use a fresh generator/cache instead of retaining it across builds. This source-workspace support does not make installed declaration-only dependencies expandable.

With explicit extra source containers, diagnostic-only programs infer their common source directory, including dependencies imported by packages inside the primary workspace. Package emit settings remain unchanged; syntax, semantic and Remote schema checks remain enabled. Without extra containers, diagnostic root handling is unchanged.
