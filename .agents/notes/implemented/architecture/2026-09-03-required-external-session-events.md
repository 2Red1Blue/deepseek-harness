...
`SessionStore.registerRequiredExternalEvents()` accepts one plugin vocabulary of namespaced event types and payload validators, and returns an idempotent disposer. A plugin owns that registration through `ctx.effect()`. `SessionStore.appendRequiredExternalEvent()` validates the detached payload through the live registration and stamps its exact identity before the immutable event commits; ordinary `Session.append()` cannot write the marker.
