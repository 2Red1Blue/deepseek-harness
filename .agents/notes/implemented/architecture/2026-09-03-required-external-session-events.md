...
`SessionStore.registerRequiredExternalEvents()` accepts one plugin vocabulary of namespaced event types and synchronous payload validators, and returns an idempotent disposer. A plugin owns that registration through `ctx.effect()`. `SessionStore.appendRequiredExternalEvent()` validates the detached payload through the live registration and stamps its exact identity before the immutable event commits; a validator that returns a Promise rejects instead of creating an unobserved asynchronous check. Ordinary `Session.append()` cannot write the marker.
