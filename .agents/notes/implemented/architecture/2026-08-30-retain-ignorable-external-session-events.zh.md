...
该生产方清单没有覆盖当前依赖此字段的一个第三方插件。没有 `ignorable` 时，第一方读取器会拒绝包含该插件信息性事件的已存会话，因为该事件不在仓库生成的 `KNOWN_SESSION_EVENT_TYPES` 中。必需外部注册不是替代方案：它标识精确且活跃的插件版本必须重建的状态变化事件，而 `ignorable` 标识读取器可以安全省略的事件。
...
[必需外部 Session 事件](2026-09-03-required-external-session-events.zh.md)机制通过精确插件注册处理状态变化的外部记录。它不改变信息性事件规则：只有读取器即使省略该事件也能正确重建时，事件才是 `ignorable`。
...
第三方信息性事件的已存记录带有显式标记时可以继续重新加载，未知必需事件则仍会明确失败。该字段继续属于公开事件信封、JSONL 表示、传输类型、生成引用及其测试；精确外部注册处理不同的状态变化类别。
