...
An external plugin whose events affect reconstruction registers its exact namespace, schema version, event types, and synchronous validators through `ctx.effect(() => ctx.sessions.registerRequiredExternalEvents(...))`, then writes only through `ctx.sessions.appendRequiredExternalEvent(...)`. The durable envelope records the registration identity; cold storage reads first validate the fixed event envelope, then require the matching active registration and validate its payload. A missing or disposed registration refuses the log, while a malformed envelope or marker, invalid payload, or asynchronous validator is corruption or a rejected write. `ignorable` applies to event types, never to unknown envelope fields; those are format-level changes. A validator receives the immutable detached payload that commits, and registration loss during validation, cold reading, or store-owned restoration refuses the operation. Only `SessionStore.prepare({ seedSource: 'persistence' })` restores a required external record; direct `Session.create()` and `Session.fromRestore()` reject it. The vocabulary is not an access-control capability: any code holding `ctx.sessions` can write an active registered type. JSONL caches the reader registration snapshot with the file revision, so an unload cannot reuse a formerly accepted log.
...
`ignorable` and `requiredExternal` are mutually exclusive. `ignorable` records that an unknown informational event may be skipped; `requiredExternal` records that an unknown state-changing event may be reconstructed only by the owning plugin version. Harness-owned event types cannot carry `requiredExternal`.
